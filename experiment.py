...
def build_workload(
    exp: Experiment, wokload_branch: str = "priv/lierseleow/loadgenerator"
):
    """
...
def setup_autoscaleing(exp: Experiment):
    ...
    for set in sets.items:
        ...
        set.spec.template.spec.containers[0].resources = client.V1ResourceRequirements(
            requests={"cpu": f'{limit["cpu"]}m', "memory": f'{limit["memory"]}Mi'},
            limits={
...
def _run_experiment(exp: Experiment, observations: str = "data/default"):
    ...
    datafile = path.join(
        observations, f"measurements_{datetime.now().strftime('%d_%m_%Y_%H_%M')}.csv"
    )
    observations_channel = FlushingQeueu(
...
def _run_remote_workload(exp: Experiment, observations: str = "data"):
    ...
    if "workload" in exp.env_patches:
        for k, v in exp.env_patches["workload"].items():
            container_env.append(client.V1EnvVar(name=f"LOCUST_{k}", value=v))
...
def cleanup(exp: Experiment):
    ...
    subprocess.run(
        ...
        cwd=path.join(env["tea_store_path"]),
    )
    subprocess.run(
